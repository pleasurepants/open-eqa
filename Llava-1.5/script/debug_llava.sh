...
#SBATCH --exclude=worker-minor-1,worker-minor-2,worker-minor-3,worker-minor-4,worker-minor-5,worker-minor-6,worker-1,worker-2,worker-3,worker-4,worker-8
#SBATCH --error=/home/wiss/zhang/code/WIP/video-blip-icl/log/error-%j.log
...
python -m debugpy --listen 0.0.0.0:8798 --wait-for-client \
 -m torch.distributed.run --nproc_per_node=1 --rdzv-id=$RDZV_ID --rdzv-backend=c10d --rdzv-endpoint=$MASTER_NODE:$MASTER_PORT \
